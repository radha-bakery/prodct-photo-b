...
let tool = "move", isPointerDown = false, lastPos = null, currentStroke = null;
let state = { rotation: 0, flip: false, zoom: 1, x: 0, y: 0, brush: 45 };
let edits = [], redoStack = [];
...
function resetState() {
  state = { rotation: 0, flip: false, zoom: 1, x: 0, y: 0, brush: 45 };
  zoomRange.value = 100; zoomVal.textContent = "100%";
  brushRange.value = 45; brushVal.textContent = "45px";
}
...
processBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    edits = []; redoStack = []; resetState(); disableEditor(false); setTool("restore");
    await renderAll();
    setStatus("Done. Restore Cut Part mode me mouse/finger drag karke missing area wapas lao.", 100);
  } catch (err) {
    console.error(err);
...
function makeComposite(canvas, showGrid=false) {
  ...
  for (const ed of edits) {
    ...
    if (!ed.points || ed.points.length === 0) continue;
    ...
    if (ed.type === "erase") {
      ...
      for (const p of ed.points) {
        ctx.beginPath();
        ctx.arc(p.x * scale, p.y * scale, ed.r * scale, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
      ...
      ctx.stroke();
      for (const p of ed.points) {
        ctx.moveTo(p.x * scale + ed.r * scale, p.y * scale);
        ctx.arc(p.x * scale, p.y * scale, ed.r * scale, 0, Math.PI * 2);
      }
      ctx.clip();
...
editCanvas.addEventListener("pointerdown", evt => {
  ...
  if (tool !== "move") {
    currentStroke = { type: tool, r: state.brush, points: [lastPos] };
    edits.push(currentStroke);
    redoStack = [];
    renderAll();
  }
});
...
editCanvas.addEventListener("pointermove", evt => {
  ...
  if (tool === "move") {
    ...
    const step = Math.max(4, state.brush / 3);
    const count = Math.max(1, Math.ceil(dist / step));

    for (let i = 1; i <= count; i++) {
      currentStroke.points.push({
        x: lastPos.x + (dx * i) / count,
        y: lastPos.y + (dy * i) / count
      });
    }
